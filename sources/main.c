...
void    get_calcimg(t_all *all)
{
    ...
    all->deps.win_ptr[1] = mlx_new_window(all->deps.mlx_ptr, all->map.x * bloc, all->map.y * bloc, "cub3Dmap");
    if (!all->deps.win_ptr[1])
        exit(1);
    ...
    {
        mlx_destroy_window(all->deps.mlx_ptr, all->deps.win_ptr[1]);
        exit(1);
    }
    data->addr = mlx_get_data_addr(data->img, &data->bits_per_pixel, &data->line_length,&data->endian);
    data2->addr = mlx_get_data_addr(data2->img, &data2->bits_per_pixel, &data2->line_length,&data2->endian);
    mlx_put_image_to_window(all->deps.mlx_ptr, all->deps.win_ptr[1], all->deps.calcs.img, 0,0);
